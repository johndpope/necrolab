...
class SaveToDatabase implements ShouldQueue {
    ...
    public function handle() {
        $data_manager = new CsvManager($this->date);
        ...
        if(!empty($files)) {
            ...
            $leaderboards_insert_queue = Leaderboards::getTempInsertQueue(10000);
            ...
            foreach($files as $lbid => $file) {    
                if($lbid != 'leaderboards') {
                    $file_handle = fopen($file['full_path'], 'r');
                    ...
                    if(Leaderboards::isValid($leaderboard, $this->date)) {
                        if(isset($ids_by_lbid[$lbid])) {
                            $leaderboard_id = $ids_by_lbid[$lbid];
                        }
                        else {
                            ...
                            $ids_by_lbid[$lbid] = $leaderboard_id;
                        }
